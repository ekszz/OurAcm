	...
	if(type == 0) {
		...
		.done(function(data) {
			...
			$.each(data.data, function(i, vo) {
				...
				reshtml = reshtml + '</td><td class="text-center inline"><div class="btn-group" id="table-toolbar-operate"><button data-aid="' + vo.aid + '" data-func="0" data-target="#activity-modal" data-toggle="modal" class="btn btn-small" title="活动详情" data-trigger="hover"' + (vo.desc == null ? ' disabled="disabled"' : '') + '>详情</button><button data-aid="' + vo.aid + '" data-func="1" data-target="#activity-modal" data-toggle="modal" class="btn btn-small" title="' + (islogin == 0 ? '请先在OJ上登录' : '报名或修改报名信息') + '" data-trigger="hover" data-placement="bottom"' + (islogin == 0 ? ' disabled="disabled"' : '') + '>报名</button><a data-aid="' + vo.aid + '" data-toggle="showlist" class="btn btn-small" title="查看已报名的同学" data-trigger="hover" data-placement="bottom"' + ((vo.ispublic == 0 && vo.adminuid != 1) ? ' disabled="disabled"' : '') + '>名单</a>' + (vo.adminuid == 1 ? ('<button data-aid="' + vo.aid + '" data-func="3" data-target="#activity-modal" data-toggle="modal" class="btn btn-small" title="导出已报名的名单为csv文件" data-trigger="hover" data-placement="bottom">导出</button>') : '') + '</div></td></tr>';
			});
